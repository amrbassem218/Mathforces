...
const Editorials: React.FunctionComponent<EditorialsProps> = ({problems, contest}) => {
  if(!contest){
    return <p>loading...</p>
  }
  return (
    <Card className='border-border max-w-240'>
      <CardHeader className="space-y-1 ">
        ...
        <CardDescription>
            All editorials are provided by {contest?.authors ? contest.authors.map((author: string, i: number) => (<span>@{author}{i+1 < contest.authors.length ? ', ' : ''}</span>)) : "@anon"}
        </CardDescription>
      </CardHeader>
        ...
        : problems.map((problem) => (
            <Card key={problem.name} className='border-border'>
              <CardHeader className=''>
                ...
                <CardDescription className='text-xs'>@{problem?.author ?? "anon"}</CardDescription>
              </CardHeader>
              <CardContent >
                {/* <div className="" /> */}
                <LimitBlur 
