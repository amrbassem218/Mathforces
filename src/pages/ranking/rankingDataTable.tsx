...
import { isRunnning, title, viewDate, viewTime } from "../../../utilities";
...
const getRankingColumns = (
  rankingData: IuserRanking[],
): ColumnDef<IuserRanking>[] => {
  return [
    {
      id: "ranking",
      accessorKey: "ranking",
      ...
      cell: ({ row }) => (
        <h2
          className={`text-center font-medium ${
            title(row.getValue("rating")).text
          }`}
        >
    ...
    },
    {
      ...
      cell: ({ row }) => <h2>{row.getValue("rating")}</h2>,
      enableSorting: true,
    },
  ];
};
